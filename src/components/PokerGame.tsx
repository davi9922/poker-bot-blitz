      ...
      <div className="relative">
        ...
        <div className="mx-auto w-[900px] h-[600px] bg-gradient-to-br from-amber-900 via-amber-800 to-amber-700 rounded-full border-8 border-amber-900 shadow-2xl relative">
          ...
          <div className="absolute inset-6 bg-gradient-to-br from-green-700 via-green-800 to-green-900 rounded-full border-4 border-green-600 shadow-inner">
            ...
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-36 bg-gradient-to-br from-green-600 to-green-700 rounded-xl border-2 border-green-500 shadow-inner">
              ...
              <div className="absolute inset-2 border border-green-400 rounded-lg bg-green-600/50">
                <div className="flex items-center justify-center h-full gap-2">
                  {communityCards.length > 0 ? communityCards.map((card, index) => <div key={index} className="transform hover:scale-105 transition-transform">
                        <PokerCard card={card} />
                      </div>) : <div className="text-green-300 text-sm font-semibold tracking-wide">
                      COMMUNITY CARDS
                    </div>}
                </div>
              </div>
            ...
            return <div key={player.id} className="absolute transform -translate-x-1/2 -translate-y-1/2" style={{
              left: `${position.x}%`,
              top: `${position.y}%`
            }}>
                  {/* Player info with enhanced styling */}
                  <div className={`mb-3 text-center ${index === 0 ? 'mb-20' : 'mt-20'}`}>
                    <div className={`flex items-center justify-center gap-2 px-4 py-2 rounded-full shadow-lg border-2 ${isCurrentPlayer ? 'bg-yellow-500 text-black border-yellow-400 animate-pulse' : player.isBot ? 'bg-red-600 text-white border-red-500' : 'bg-blue-600 text-white border-blue-500'}`}>
                    ...
                    <div className="mt-2 flex flex-col items-center gap-1">
                      ...
                      {player.currentBet > 0 && <div className="flex items-center gap-1">
                  ...
                  <div className="flex gap-2 justify-center">
                    {player.hand.map((card, cardIndex) => <div key={cardIndex} className="transform hover:scale-105 transition-transform">
                        <PokerCard card={showdown || !player.isBot ? card : null} faceDown={!showdown && player.isBot} />
                      </div>)}
                  </div>
                  
                  {/* Folded indicator */}
                  {player.isFolded && <div className="absolute inset-0 bg-black/50 rounded-lg flex items-center justify-center">
            ...
            {/* Turn indicator */}
            {gameState === 'playing' && !showdown && <div className="absolute top-6 left-6">
                <div className={`px-4 py-2 rounded-full text-sm font-bold shadow-lg ${playerTurn ? 'bg-blue-600 text-white animate-pulse' : 'bg-gray-600 text-white'}`}>
        ...
        <div className="mt-8 text-center space-y-4">
          {/* Game result */}
          {winners.length > 0 && <div className={`text-2xl font-bold p-4 rounded-lg inline-block shadow-lg ${winners.includes(0) ? 'bg-green-600 text-white' : 'bg-red-600 text-white'}`}>
              {winners.includes(0) ? '🎉 ¡GANASTE!' : '😔 PERDISTE'}
              {winners.length > 1 && ' (EMPATE)'}
            </div>}

          {/* Action buttons with enhanced styling */}
          {gameState === 'playing' && playerTurn && !showdown && <div className="flex flex-wrap justify-center gap-4">
              {canCheck && <Button onClick={handleCheck} variant="outline" className="bg-green-600 text-white border-green-500 hover:bg-green-500 shadow-lg">
                  ✅ Check
                </Button>}
              {canCall && <Button onClick={handleCall} className="bg-blue-600 hover:bg-blue-500 shadow-lg">
                  📞 Call ({currentBet - (players[0]?.currentBet || 0)})
                </Button>}
              <div className="flex items-center gap-3 bg-white rounded-lg p-2 shadow-lg">
                <input type="number" value={betAmount} onChange={e => setBetAmount(Math.max(gameConfig.bigBlind, parseInt(e.target.value) || gameConfig.bigBlind))} min={gameConfig.bigBlind} max={playerChips} className="w-24 px-3 py-2 rounded border text-center font-semibold" />
                <Button onClick={handleRaise} disabled={!canRaise} className="bg-red-600 hover:bg-red-500 shadow-lg">
                  🚀 Raise
                </Button>
              </div>
              <Button onClick={handleFold} variant="destructive" className="shadow-lg">
                💔 Fold
              </Button>
            </div>}
          ...
          <div className="flex justify-center gap-4">
            {gameState === 'waiting' && <Button onClick={dealNewHand} size="lg" className="bg-green-600 hover:bg-green-500 shadow-lg">
                🎯 Repartir Cartas
              </Button>}
            {(gameState === 'finished' || showdown) && <Button onClick={dealNewHand} size="lg" className="bg-green-600 hover:bg-green-500 shadow-lg">
                🔄 Nueva Mano
              </Button>}
            <GameSettings config={gameConfig} onConfigChange={handleConfigChange} onResetGame={resetGame} />
            <Button onClick={resetGame} variant="outline" size="lg" className="border-white text-white hover:bg-white hover:text-green-800 shadow-lg">
              🎮 Reiniciar Juego
            </Button>
          </div>
        </div>
